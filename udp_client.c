...
int main(void){
    ...
    while (1){
        switch(state){
            case 0: printf("Enter message 0: ");
                    ...
                    state=1;
                    break;
            case 1: if(recvfrom(s,&rcv_ack,sizeof(rcv_ack),0,(struct sockaddr *) &si_other,slen)==-1){
                    die("recvfrom()");
                    }
                    if(rcv_ack.sq_no==0){
 ...
 //wait for sending packet with seq. no. 1
                    ...
                    state = 3;
                    break;
            case 3: if(recvfrom(s, &rcv_ack, sizeof(rcv_ack), 0, (struct sockaddr *) &si_other, &slen) == -1)
                    {
                    die("recvfrom()");
                    }
                    if (rcv_ack.sq_no==1)
