...
int main(){
    ...
    printf ("%s\n", msg);
    printf ("Doing +1 \n");
    // Convert received string to a float or double
    double realNumber = atof(msg);

    // Calculate the next higher integer
    int nextInteger = (int)realNumber + 1;

    // Send the next higher integer back to the client
    snprintf(msg, sizeof(msg), "%d", nextInteger);
    // gets (msg);
